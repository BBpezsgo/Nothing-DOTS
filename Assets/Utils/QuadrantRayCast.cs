...
public static class QuadrantRayCast
{
    ...
    static bool RayCast_(
        ...
        out Hit hit)
    {
        var ray3 = new Ray(new float3(ray.Start.x, 0f, ray.Start.y), new float3(ray.End.x, 0f, ray.End.y), ray.Layer, ray.ExcludeContainingBodies);
        for (int i = 0; i < entities.Length; i++)
        {
            if ((entities[i].Layer & ray.Layer) == 0u) continue;
            ...
            if (ray.ExcludeContainingBodies && CollisionSystem.Contains(
                    entities[i].Collider,
                    new float3(entities[i].Position.x, 0f, entities[i].Position.z),
                    ray3.Start))
            { continue; }
